...
class EquipesJuniorDAO extends AbstractDAO {
    protected static $tableName = "equipes_juniors";
    protected static $primaryKey = "id_equipes_juniors";

    protected function createObject($data) {
        return new Equipe($data['id_equipes_juniors'], $data['nom_equipes_juniors']);
    }

    protected function getInsertFields() {
        return "nom_equipes_juniors";
    }
    ...
    public static function obtenirToutesLesEquipes() {
        try {
            $connexion = self::getConnexion();
            $requete = $connexion->prepare(
                "SELECT * FROM " . self::$tableName . " ORDER BY nom_equipes_juniors"
            );
            $requete->execute();
